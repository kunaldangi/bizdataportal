    ...
    Mutation: {
        removeTableUser: async (_root: any, data: {tableId: number, userId: number}, context: Context): Promise<TableUser> => {
            ...
            try{
                ...
                if(!contextTablePrm?.manageUsers && !context.permissions.tables.manageUserPermissions) throw new GraphQLError('You do not have permission to manage users in this table.'); // u need admin permission or table manage user permission
                ...
                if(!usrTablePermReuslt) throw new GraphQLError('User not found in table!');
