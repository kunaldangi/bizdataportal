...
export const resolver = {
    ...
    Mutation: {
        addWhitelistEmail: async function addWhitelistEmail(_root: any, {email}: {email: String}, context: Context): Promise<Whitelist>{       
            ...
            try {
                if(!context?.permissions?.usersAcc?.manageWhitelist && !(process.env.SERVER_RUN_MODE === "development")) throw new GraphQLError('You do not have permission to manage whitelist.');
